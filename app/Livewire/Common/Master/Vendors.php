...
class Vendors extends Component
{
    ...
    public function mount()
    {
        ...
        $this->types = VendorTypes::where('soft_delete', 0)
            ->where('status', 1)->pluck('name', 'vendor_type_id');
    ...
    $items =$this->model::query()
        ...
        ->when($this->vehicle, fn($q) =>
            $q->whereHas('vehicles', fn($qr) =>
                $qr->where('vehicle_id', $this->vehicle)
            )
        )
    
        ->when($this->location, fn($q) =>
            $q->whereHas('serviceLocations', fn($qr) =>
    ...
    public function edit($id)
    {
        ...
        $this->updatedTypeId($item->type_id);
    ...
    public function resetForm()
    {
        $this->reset([
            ...
            'type_id',
            'notes',
            'service_area_id','selectedServiceArea','serviceAreaNames'
        ]);
    ...
    public function updatedTypeId($id)
    {
        $this->is_taxi = VendorTypes::where('vendor_type_id', $id)
            ->value('is_taxi');
    }
