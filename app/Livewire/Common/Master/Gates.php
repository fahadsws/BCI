...
class Gates extends Component
{
    ...
    public $isadd = false;
    public $pageTitle = 'Gates of the park';
    ...
    public function updatedPark($id){
        $this->zones = Zones::orderBy('zone_id', 'desc')
            ->where('park_id',$id)
            ->pluck('name', 'zone_id')
            ->toArray();
    ...
    {
        $this->total_week_day = $this->gypsy_charge + $this->guide_fee + $this->weekday_permit;
    }
    public function updatedWeekendPermit()
    {
        $this->total_week_end = $this->gypsy_charge + $this->guide_fee + $this->weekend_permit;
    }
